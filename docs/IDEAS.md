...
shared FOC data set, state = deterministic fold, ~30 to 60 seconds per move, no
server anywhere.

## Games that fit 30 to 60 seconds moves

1. **Connect-4 / checkers**: flashy enough to screen-record, fold logic
   still trivial. Connect four shipped (`games/connect-four/`); checkers
   is open.
...
- Piece-body cache to IndexedDB (localStorage quota ~5 MB).
