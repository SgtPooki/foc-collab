/**
 * BYOW fold (schema v2): seat-owner sequencing over per-player data sets.
 * Pure and synchronous. No I/O, no clocks, no hashing.
 *
...
export function initialByowState(game, root) {
  return {
    ...initialState(game),
    v: V,
    root: String(root),
...
/** Folds pieces from any number of data sets into one game's state. */
export function foldByow(game, root, pieces) {
  const rootId = String(root)
  let state = initialByowState(game, rootId)
  ...
  for (const move of ratifications) {
    ...
    if (!joinMatchesRatification(join, move) || !legalCell(state, move.cell)) continue
    state = {
      ...state,
...
export function lobbyByow(pieces) {
  ...
  return [...roots.values()].map(({ game, root }) => foldByow(game, root, pieces))
}
