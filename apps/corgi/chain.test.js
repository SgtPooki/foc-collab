...
function fakeClient({ head, logs, maxRange = 10_000 }) {
  ...
  return {
    ...
    async getLogs({ fromBlock, toBlock, args }) {
      ...
      return logs
        .filter((l) => l.blockNumber >= fromBlock && l.blockNumber <= toBlock && l.args.to === args.to)
        .map((l) => ({ ...l }))
    },
  }
}
const log = (from, amount, block, logIndex = 0, txHash = `0xtx${block}_${logIndex}`) => ({
  args: { token: calibration.contracts.usdfc.address, from, to: PAYER, amount }, blockNumber: BigInt(block), logIndex, transactionHash: txHash,
})
...
test('scans in chunks, returns deposits oldest first with fold-ready fields', async () => {
  ...
  assert.equal(client.calls.length, 3) // 24001 blocks in 10k chunks
