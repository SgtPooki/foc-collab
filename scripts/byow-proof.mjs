 ...
 *   node scripts/byow-proof.mjs .byow/player-a.json .byow/player-b.json
 *
 * Descriptor files come from scripts/byow-setup-player.mjs. O's data set
 * is found by X through chain events: O's join upload is tagged with the
 * game id and X scans PieceAdded from the create block. No lobby, no
 * publisher key.
 */
...
if (!aPath || !bPath) {
  console.error('usage: node scripts/byow-proof.mjs <player-a.json> <player-b.json>')
  process.exit(1)
}
...
async function client(name, me, peers) {
  ...
  return {
    ...
    async view(game, root, from = null) {
      if (from != null) {
        const { failed } = await transport.discover({ app: APP, game, from })
        if (failed.length > 0) log(name, 'scan gaps', failed)
      }
    ...
    async write(payload, label) {
      ...
      await transport.append(signed, (stage) => log(name, `${label}: ${stage}`), tagsFor(APP, payload.game, payload.type))
...
async function until(who, game, root, label, predicate, from = null, timeoutMs = 6 * 60_000) {
  const started = Date.now()
  for (;;) {
    const state = await who.view(game, root, from)
    if (predicate(state)) {
...
log('proof', `wallet A ${A.wallet} data set ${A.ds}; wallet B ${B.wallet} data set ${B.ds}; discovery via PieceAdded events`)
...
const from = String(await alice.transport.blockNumber())
log('proof', `game ${game} rooted in data set ${root}; invite start block ${from}`)

// 1. X creates in X's data set. The invite is (game, root, from).
...
// 2. O follows the invite: reads the root, joins in O's own data set (tagged upload).
...
// 3. X discovers O's data set from the PieceAdded event and ratifies with the first move.
a = await until(alice, game, root, 'alice discovers a joiner via chain events', (s) => s.joins.length > 0, from)
assert.deepEqual(alice.transport.dataSets().sort(), [root, String(B.ds)].sort(), 'discovery added exactly O\'s data set')
...
// wallet or key reconstructs the game from FOC alone, no event scan needed.
const carol = await client('carol', null, [root])
